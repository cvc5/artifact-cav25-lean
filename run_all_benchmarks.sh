...
# Run the 'Seventeen' suite benchmarks
...
# Run the SMT-LIB suite benchmarks
run_smtlib_benchmarks() {
  local smt_file="$1"

  python3 run_benchmarks.py "$smt_file" cvc5+leansmt --jobs "$JOBS" --timeout 60 --memout 8192
  ...
  python3 run_benchmarks.py "$smt_file" cvc5+ethos --jobs "$JOBS" --timeout 60 --memout 8192
...
# Run the benchmarks
run_seventeen_benchmarks "$SEVENTEEN_SMT2_FILE" "$SEVENTEEN_FOF_FILE"
run_smtlib_benchmarks "$SMTLIB_FILE"
